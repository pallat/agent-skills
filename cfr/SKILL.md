...
Mandatory standards for every Go service repository. Items marked *(Adv)* are recommended.
...
- Block deprecated modules via linter config.
- Use modern features (generics, `range over func`, `copyloopvar`) where they help.
...
- `internal/` holds domain + adapters; not importable externally.
- `pkg/` holds reusable tech packages; never imports `internal/`.
- Clean-arch repos: domain imports stdlib only; adapters are domain subpackages implementing `port.go` interfaces.

## 4. Dockerfile
- Multi-stage: builder compiles, runtime runs binary.
- Runtime image minimal (Alpine or equiv) with init process (`tini`/`--init`) for signal handling.
- Non-root user — create dedicated user and switch.
- `HEALTHCHECK` pointing at liveness endpoint.
- `COPY --link`, `--mount=type=cache` for module cache.
- Static binary: `CGO_ENABLED=0 go build -trimpath -ldflags="-s -w"`.
- Embed commit hash via build arg + `-ldflags -X`.
...
- Include observability keys (service name, exporter endpoint, insecure flag), `LOG_LEVEL`, health probe config.
- Every variable maps to a config struct field — no orphans.
- TLS/SASL defaults must be safe (disabled).
...
## 9. Observability
- OTel SDK (traces + metrics) in a reusable package.
- Structured logging injects `trace_id` for correlation.
...
## 10. OpenAPI *(HTTP only)*
- Spec, generated Go code, and codegen config in a dedicated directory.
- `make openapi-gen` regenerates; `make openapi-check` fails CI on stale code.
- Codegen tool version pinned.
...
- CI Go version must match `go.mod`.
- *(Adv)* OpenAPI spec drift check.

## 12. README
- Document architecture layout, dependency rules, quickstart, observability section, structure tree.
- List env vars referencing the env template.
- Repo name in README must match actual repo name.

## 13. CHANGELOG & CONTRIBUTING
- CHANGELOG has a dated latest entry.
- CONTRIBUTING covers dev setup, pre-commit steps, how to add a domain package.
- Clean-arch repos: CONTRIBUTING explains architecture rules.

## 14. Scripts
- Setup and version-bump scripts exist and are executable.
- *(Adv)* Dev dep installer, pre-commit setup, commit-msg convention, test colorizer.
- No script contains hardcoded secrets.

## 15. Security
- No hardcoded credentials anywhere.
- Docker non-root; no `InsecureSkipVerify: true` in TLS config.
- Env template has placeholders only; DLQ/error headers must not leak sensitive data.
- `govulncheck` passes.
- *(Adv)* SOPS/age env encryption, `gosec` in linter, secret detection in CI.

## 16. Version & Release
- `VERSION` file using semver (`vX.Y.Z`).
- Version in Docker label and health probe response.
- `make bump-version` updates VERSION and creates git tag.

## 17. docker-compose
- Include only services needed for local dev.
- Service/cluster names reference repo name.
- *(Adv)* Volume mounts for persistence; network isolation.

## 18. HTTP Status Convention *(HTTP only)*
- 14 codes: 200/201/204, 400/401/403/404/409/422/429, 500/502/503/504.
- 4xx = no retry, 5xx = retry.
- Every error body includes `error_code` field.
